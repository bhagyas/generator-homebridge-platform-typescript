...
declare var Accessory, Service, Characteristic, UUIDGen;
...
function SamplePlatform(log, config: any, api?){
  log("SamplePlatform Init");
  var platform = this;
  this.log = log;

  //loads the configuration provided to homebridge platform.
  this.config = config;
  this.accessories = [];
  ...
  //api would be empty if launched from an older homebridge version.
  if (api) {
...
SamplePlatform.prototype.addAccessory = function(accessoryName: string) {
